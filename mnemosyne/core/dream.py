...
def _unexpected_dream_failure(
    beam, run_id: str, exc: Exception, *, persist: bool,
) -> DreamRun:
    """Classify an unexpected ``Exception`` from a public Dream API boundary.

    Returns a static ``DreamRun`` whose (state, error_code, failure_reason)
    triple is drawn only from the taxonomy below, never from ``exc``. The
    exception text is read transiently for the locked/busy predicate and then
    discarded.

    * ``sqlite3.OperationalError`` whose message indicates busy/locked is
      classified as ``failed_retryable`` / ``database_busy``.
    * Every other ``Exception`` is ``failed_terminal`` / ``integrity_failure``.

    When ``persist=True`` and ``run_id`` is a nonempty identifier of an
    existing row, a best-effort attempt is made to persist the failure on the
    durable ``dream_runs`` row (init schema -> set state -> commit -> reload).
    If any secondary operation fails or the reload cannot safely deserialize,
    the synthetic static ``DreamRun`` is returned. When ``persist=False`` no
    schema init / state set / commit is attempted; only a best-effort rollback
    guards the boundary. Every step is guarded so even a rollback or database
    fault cannot escape the boundary.
    """
    msg = str(exc).lower()
    if isinstance(exc, sqlite3.OperationalError) and (
        "locked" in msg or "busy" in msg
    ):
        state, code = "failed_retryable", "database_busy"
    else:
        state, code = "failed_terminal", "integrity_failure"
    ...
    if persist and run_id:
        try:
            ...
            _set_state(
                beam.conn, run_id, state,
                error_code=code, failure_reason=code,
            )
            beam.conn.commit()
            reloaded = _load_run(beam, run_id)
    ...
    return DreamRun(
        run_id=run_id, state=state, error_code=code, failure_reason=code,
    )
...
def _dream_plan_impl(
    beam, scope: Dict[str, Any], run_id: str,
    ...
    ``run_id`` is generated by the public :func:`dream_plan` wrapper so the
    structured caller-transaction rejection and any pre-persistence failure
    carry a real UUID even when no durable row is ever written.
    """
...
def dream_plan(
    beam, scope: Dict[str, Any], limits: Optional[Dict[str, Any]] = None,
    request_id: Optional[str] = None,
) -> DreamRun:
    """Public boundary for :func:`_dream_plan_impl`.

    Generates the run UUID up front so a pre-persistence failure still returns
    a real identifier. On an unexpected failure, the synthetic static result
    keeps the valid UUID without leaking exception text.

    # ponytail: plan failures without a durable row are in-memory; add an
    # INSERT-only audit row only if operators require durable failed-plan
    # history.
    """
    run_id = str(uuid.uuid4())
    try:
        return _dream_plan_impl(beam, scope, run_id, limits, request_id)
    except Exception as exc:
        return _unexpected_dream_failure(beam, run_id, exc, persist=False)
...
def _dream_submit_receipt_impl(beam, run_id: str, receipt: Any) -> DreamRun:
...
def dream_submit_receipt(beam, run_id: str, receipt: Any) -> DreamRun:
    """Public boundary for :func:`_dream_submit_receipt_impl`.

    An unexpected failure (e.g. a fault between the receipt INSERT and the
    state UPDATE) is contained as a static failure on the durable row; the
    receipt insert is rolled back by the best-effort guard.
    """
    try:
        return _dream_submit_receipt_impl(beam, run_id, receipt)
    except Exception as exc:
        return _unexpected_dream_failure(beam, run_id, exc, persist=True)
...
def _dream_apply_impl(beam, run_id: str) -> DreamRun:
...
def dream_apply(beam, run_id: str) -> DreamRun:
    """Public boundary for :func:`_dream_apply_impl`.

    An unexpected failure that escapes the impl (including a preflight schema
    init fault or secondary database fault) is contained as a static failure
    on the durable row.
    """
    try:
        return _dream_apply_impl(beam, run_id)
    except Exception as exc:
        return _unexpected_dream_failure(beam, run_id, exc, persist=True)


def _dream_undo_impl(beam, run_id: str) -> DreamRun:
...
def dream_undo(beam, run_id: str) -> DreamRun:
    """Public boundary for :func:`_dream_undo_impl`.

    An unexpected failure that escapes the impl is contained as a static
    failure on the durable row.
    """
    try:
        return _dream_undo_impl(beam, run_id)
    except Exception as exc:
        return _unexpected_dream_failure(beam, run_id, exc, persist=True)
...
def _dream_resume_impl(beam, run_id: str) -> DreamRun:
    ...
    Calls the PUBLIC ``dream_status`` and ``dream_apply`` so each inner
    boundary call is independently contained by its own wrapper.
    """
    rejection = _reject_if_caller_holds_transaction(beam, run_id)
    if rejection is not None:
        return rejection
...
def dream_resume(beam, run_id: str) -> DreamRun:
    """Public boundary for :func:`_dream_resume_impl`.

    An unexpected failure during the status/apply chain is contained as a
    static terminal result without re-raising.
    """
    try:
        return _dream_resume_impl(beam, run_id)
    except Exception as exc:
        return _unexpected_dream_failure(beam, run_id, exc, persist=True)


def _dream_status_impl(beam, run_id: str) -> DreamRun:
...
def dream_status(beam, run_id: str) -> DreamRun:
    """Public boundary for :func:`_dream_status_impl`.

    Read-only API: a failure is never persisted (``persist=False``).
    """
    try:
        return _dream_status_impl(beam, run_id)
    except Exception as exc:
        return _unexpected_dream_failure(beam, run_id, exc, persist=False)
